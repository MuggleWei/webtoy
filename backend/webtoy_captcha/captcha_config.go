...
func InitConfig() (*CaptchaConfig, error) {
	// pflag
	pflag.String("host", "0.0.0.0", "bind host")
	pflag.Uint("port", 8080, "listen port")

	pflag.String("log.level", "info", "log level")
	pflag.String("log.file", "./log/webtoy-captcha.log", "log file path")
	...
	pflag.String("sr.service.name", "webtoy-captcha", "service name")
	pflag.String("sr.service.id", "webtoy-captcha-0", "service id")
	...
	pflag.String("session.expired", "1m", "session expired time duration")
	...
	viper.AddConfigPath("/etc/webtoy_captcha")
...
func PrintConfig(conf *CaptchaConfig) {
	log.Info("--------------------")
	log.Info("captcha config:")
